...
from typing import Optional, Dict, FrozenSet, List, Tuple, Any
import jsonpickle
...
class Session(commands.Bot):
    _lib: RuntimeLibrary
    _players: Dict[str, PlayerData]

    def __init__(self, command_prefix, self_bot, runtime_library: RuntimeLibrary, players: List[PlayerData]):
        commands.Bot.__init__(self, command_prefix=command_prefix, self_bot=self_bot)
        self._lib = runtime_library
    ...
    def from_save_file(filepath: Path) -> Session:
        ...
        return Session(">", False, RuntimeLibrary(lib), p_data)
...
bot = Session.from_save_file(Path(os.path.dirname(__file__)).parent / 'savefile_2.zip')
bot.run(TOKEN)
